...
If !IsObject(AutoReplace)
	{
	 IfExist, % Config["ClipDataPath"]["AutoReplace"]
		{
		 If (XA_Load(Config["ClipDataPath"]["AutoReplace"]) = 1) ; the name of the variable containing the array is returned OR the value 1 in case of error
			{
			 MsgBox, 16, AutoReplace, AutoReplace.xml seems to be corrupt, starting a new empty AutoReplace.xml.
			 FileDelete, % Config["ClipDataPath"]["AutoReplace"]
			 AutoReplace:=[]
			}
		}
	 else
